...
function zoom(chart, amount, transition = 'none') {
  ...
  helpers.callback(state.options.zoom.onPreZoom, [{chart}]);
...
function resetZoom(chart, transition = 'default') {
  ...
  helpers.callback(state.options.zoom.onPreZoomComplete, [{chart}]);
  chart.update(transition);
...
function getZoomLevels(chart) {
  ...
  helpers.each(chart.scales, function(scale) {
    ...
    if (origRange) {
      ...
      min = Math.min(min, level);
      max = Math.max(max, level);
      toReturn[scale.id] = min < 1 ? min : max;;
    }
  });
  return toReturn;
}
...
var Zoom = {
  ...
  start: function(chart, _args, options) {
    ...
    chart.getZoomLevels = () => getZoomLevels(chart);
